...
int main(int argc, char** argv) {
   ...
   inputPlayers(); // get number of players and populate a list
   display(); // display that newly populated list
   inputRhyme(); // get an array from the user

   int x = 1;
   while (last->link != last) {
	  printf("\n*** ROUND #%d ***\n", x);
	  doRhyme(); // perform one round of the game
	  x++;
   }
